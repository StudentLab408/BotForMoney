...
def test_official_docx_has_full_amount_and_no_internal_columns():
    doc = Document(build_supplement_docx(2026, 9, _rows(), official=True))
...
def test_internal_docx_has_lab_share_basis_and_totals():
    doc = Document(build_supplement_docx(2026, 9, _rows(), official=False))
    table = _table_text(doc)
...
def test_empty_list():
    doc = Document(build_supplement_docx(2026, 1, [], official=True))
