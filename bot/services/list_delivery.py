...
from bot.utils.time import from_period, period_title
...
async def send_monthly_lists(
    bot: Bot, chat_id: int, period: int, rows: list[DocRow], *, caption_prefix: str = ""
) -> None:
    ...
    for official, suffix, caption in VARIANTS:
        buf = build_supplement_docx(period, rows, official=official)
        await bot.send_document(
            chat_id,
            BufferedInputFile(buf.getvalue(), filename=f"nadbavki_{year}_{month:02d}_{suffix}.docx"),
            caption=caption_prefix + caption.format(month_name=period_title(period)),
        )
